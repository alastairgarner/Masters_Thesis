---
#
title: "Identification of premotor components for nocifensive behaviour in Drosophila larvae"
author: 
	- Alastair Garner
date: 18/03/2020
...
Locomotion is a rhythmic and highly coordinated behaviour that enables 

Recruitment of common set of MNs facilitates performance of a number of unique behaviours.

Requires exquisite control over the activation of specific motor pools

Presumably is the role of the premotor circuitry.



How connectivity of PMNs facilitate the activation of specific movements/behaviours remains largely unanswered



Apart from *C. elegans* no system has been able to identify and characterise, in connectivity and function, all PMNs and MNs. Open question on whether behaviour-specific neuron populations exist as low as directly premotor neurons.

Here we make initial inroads toward characterising 

Identify neurons that affect the performance of 
...
**NEURON TRACING + SUMMARISE**



Jenett 2012 - Original GAL4 lines

## *Drosophila* larval nocifensive behaviour



*Drosophila* larva have emerged as a powerful model to study nociception and nocifensive behaviour over the last 20 years. In their natural environment *Drosophila* are preyed upon by parasitoid waps [@Carton1986] like those of the *Chalcidoidea* and *Ichneumonoidea* superfamilies. The female wasp inserts its sharp ovipositior through the larval cuticle and lays an egg. The developing wasp larva then digests the host *Drosophila* from the inside out, eventually resulting in the pupation and eclosion of a juvenile wasp. Parasitoid wasps produce a strong selective pressure on *Drosophila*[@Powell1997], leading to the evolution of endogenous mechanisms to fight the parasite. The first of these is a cellular immune response that can destroy the wasp egg, preventing the development of wasp and thus also the death of the *Drosphila* larva[@Rizki1990]. The other is a unique behavioural mode in which the *Drosophila* larva produces a corkscrew-like "roll" motion, where the larva rotates around the anteroposterior axis[@Tracey2003]. This direction of a roll is dictated by the position at which the wasp ovipositor is inserted, such that the rotation of the larval body removes the ovipositor, preventing egg-laying[@Hwang2007]. In laboratory conditions rolling behaviour can be readily elicited by activation of the *painless* expressing class IV dendrite arborization (CIVda or MDIV) neurons, a class of multimodal nociceptors that respond to harsh mechanical stimulation[@Zhong2010], noxious temperatures[@Zhong2012] and strong UV light[@Xiang2010].
...
**Describe full repertoire**

**Replace with image timelapse for each behaviour**

![](./escape_repertoire.svg)
...
We know that 4 of the 13 inputs downstream of can activate roll



## Quantifying Escape Behaviours

**Maybe should place earlier**
...
Talk about direct vs indirect innervation nature of PMSIs
...
Kaneko 2017 - serotonergic modulation of MD IVs - by A08n?
Schneider-Mizell 2016
Gerhard 2017
Carreira-Rosario 2018

Relative importance of MNs and Muscles in each behaviour is somewhat unknown, but we do know the order of contraction (Heckscher). Should I talk about the pattern of contraction here? Something on motifs coordinating muscle contraction?
...
Something about steps to understanding the black box of premotor circuitry 
...
**Aim 2:** Characterise phenotypic contribution of candidate neurons populations to locomotor behaviours

**Hypothesis 2:** Direct premotor neurons are likely implicated in the performance of all larval locomotor behaviours, where indirect premotor neuron may be behaviour-specific

***

**Aim 3:** Evaluate the 

**Hypothesis 3:** Interneuron populations with similar anatomical connectivity and and sign of neurotransmission (excitatory/inhibitory) will affect similar components of the nocifensive behaviour repertoire. Perturbing the function of these neurons will manifest in a phenotypically similar manner.

***

**Aim 4:** Evaluate the performance of 

**Hypothesis 4:** Defining discrete behaviours (e.g. rolling) is subjective by nature, so data processing pipelines of different design may report said behaviours differently. If the phenotype observed in our candidate lines is truly robust, it should be evident regardless of which pipeline is used for analysis. 
...
in training the classifier, we defined a 'roll' as:. Manual scoring of behaviour was done to this definiton.
...
Statistical analyses were performed using MATLAB (MathWorks) software. For our initial screen, the proportions of animals that performed a behaviour (roll) were compared between genotypes using the Fisher's Exact test. Comparison of other metrics were conducted using the Mann-Whitney U test (Wilcoxon rank-sum test). Where multiple comparisons were made, *p* values were corrected using the Bonferroni correction.
...
## Screening Interneuron Populations for Involvement in Rolling Behaviour
...
We used a high-throughput optogenetic assay to determine which, if any, of a catalgoue of interneuron expression lines might be important for the performance of rolling behaviour. We employed optogenetics to coactivate both the Basin neurons, to induce nocifensive behaviour, as well as one of the population labelled by the split-GAL4 lines[@ref]. We compared the performance of rolling for 135 different split-GAL4 lines to a positive control (*attp2;;72F11-GAL4,UAS-CsChrimson*) and evaluated the performance of rolling based on the metrics generated by the Salam pipeline for scoring behaviour[@Ohyama2013]. 135 split-GAL4 lines (which we refer to by the last four digits in the name) were tested, each with approx 150 animals across an average of 4 repeats over multiple days. For statistical analysis experiments were only compared to those controls performed on the same-day to account for day-to-day variance. **Some figures are normalised** by control values, such that the resultant metric is the **difference ($\Delta$) between the experimental and same-day control**.

At an initial light intensity of ???W/cm<sup>2</sup> rolling reliably induced rolling in 90% of control animals per-trial. If the neuron populations being tested were important or necessary for rolling behaviour, we expected that we might see a decrease in the incidence of rolling. Only one line tested gave a significant decrease in the proportion of animals that performed rolling behaviour (4169), when compared to the same-day control. Interestingly, despite the high baseline of roll proportion in controls we did observe several lines that showed an increase in roll proportion. We note that other lines that increase roll proportion may have been occluded, given said high baseline.

![](./Thesis_Figure_1.svg)

***

To assess the day-to-day variability in data we analysed the performance of control animals across each trial. It is understood that the likelihood of rolling varies with larval age and, by extension, larval size (supplemental). As such we restricted experiments to third instar larva and ignored animals with an average area < 2.5mm<sup>2</sup> in post-hoc analyses, below which roll likelihood decreases. Despite this, we saw a range of larval size across our control experiments (fig ref), with median values from 2.5 to 5mm<sup>2</sup>. Likewise we observed a large degree of variation in the the per-animal time-spent-rolling and number of rolls between trials. Such variation may confine statistical analyses versus our experimental lines. 

These analyses of variation have been useful post-hoc, but were not available when informing which lines to pursue. They shall be discussed later in this thesis.

To assess changes in the presentation of rolling behaviour we analysed the average time spent rolling by individual. We observed that 65 of the lines tested showed a significant increase in the time spent rolling, while 6 showed a significant decrease (rig ref). Further we found many lines showed statistically significant increases and decreases in the number of rolls per animal and the average amplitude of rolls per animal (supplemental), where the amplitude of a roll is defined as the maximum crabspeed during a bout of rolling. The abundance of significant results should be viewed cautiously, as it likely results from the lofty statistical power (insert value here) achieved with *50 < n < 250* per genotype. 

Whilst we observed changes in the number, duration and amplitude of roll bouts understanding how these changes manifest in the posture and movement of the larva requires analysis of lower-level metrics. Rolling behaviour is characterised predominantly by high curvature along the "spine" (midline from head to tail) coincident with elevated crabspeed (movement perpendicular to the body axis) (**need explanatory figure here**). Given that roll bouts are scored determinant on crabspeed see Methods) it is unsurprising that most of the lines presenting increased roll duration also show increased average crabspeed during stimulation. Notably the average curvature of the animals does not copresent with increased rolling, with 1816 and 1817 showing significantly decreased curvature (p < 0.005) during the bout of stimulation.

![](./Thesis_Figure_2.svg)

***

Given the plethora of significant results and the relative variability of our control data it was important to asses changes to rolling at a finer timescale, as well as on an individual basis, before selecting lines to study further. We ignored all lines where either the experimental larva or same-day controls had n < 50 and sought to eliminate other lines based on uncharacteristic control data.

To do this we visualised the previously mentioned metrics over the time series of the experiment. Lines presenting with increased roll proportions generally fell into two categories. The first presented with a delayed but increased peak in roll percentage 2 seconds after the onset of stimulation. Roll percentage steadily decreased, yet remained higher than the control, throughout the remainder of stimulation. While 1816 showed a significant increase in crabspeed during stimulation (p < 0.005) the temporal profile of crabspeed showed only a marginal increase, limited to the first 5 seconds of stimulation. Otherwise 1816 and 1817 were indistinguishable from the control. However, both 1816 and 1817 did demonstrate a marked decrease in curve during stimulation.

The second group (0666,1750, 4052, 4232) showed a strong biphasic response with an initial spike in roll percentage within 1 second of stimulation, followed by a delayed and prolonged increase in roll percentage after ~5 seconds. For 0666, 4052 and 4232 this was copresented with and marked increase in crabspeed during the stimulation bout. All lines showed a temporal profile in curve similar to that of the control.

Another group of lines (1951, 4245, 4189) presented a decrease in roll percentage during the late phase of the stimulation bout. This was complemented by a decrease in both the curve and the crabspeed of the larvae throughout the course of stimulation. We noted that the control data was higher than usual for these particular comparisons, perhaps distorting the significance of these results. As such we did not follow up on these particular lines.

The final line of interest, 4248, displayed no obvious phenotype for rolling behaviour, but failed to display the stereotyped 'fast-crawl' behaviour that follows a bout of rolling. Analysis of larval speed in control data shows that, at the end of a bout of stimulation, speed increases above the pre-stimulation baseline. Larval speed in 4248 only increased to that of baseline. (need separate figure for this - boxplot).

![](./Thesis_Figure_3.svg)

Our data suggested that we might have found candidate lines that are implicated in the performance of larval nocifensive behaviour. To test the robustness of these responses we choose to repeat the experiment at a reduced light intensity (???W/cm<sup>2</sup>). Basin-activated nocifensive behaviours present differently at different stimulation intensity (data not shown). At high intensities rolling behaviours are sustained throughout a period of stimulation, whereas at lower intensities larval switch to crawling behaviours after 5-10s (figure). Therefore we wanted to know whether the observed phenotypic effects were consistent, or unique to the high-power activation. 

Consistent with our original screen 1816 and 1817 showed an increase in the total time-spent rolling. While 1816 also had a significant decrease in the average curve during stimulation, neither 1816 or 1817 appeared markedly different over the time series of stimulation. 1750 and 4232 also showed consist results with the original screen, demonstrating an increase in roll percentage and crabspeed across the bout of stimulation. In contrast to the initial screen 4052 and 0666 did not present an increase in either roll percentage or crabspeed. Again we note that there is a large variation in the performance of control data, most evidently between the controls run with 4232 and 1750 versus 0666 and 4052, with an 8-fold increase in the maximum roll percentage and 2-fold increase in average crabspeed. Further repeats will be necessary to determine the validity of these results.
